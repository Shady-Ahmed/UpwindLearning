# -*- coding: utf-8 -*-
"""
Data generation for 1D Burgers problem with an initial condition of pulse
...
for ii in range(len(pw)):
    ...
    #compute initial conditions [pulse]
    uu = np.zeros(nx+1)
    for i in range(nx+1):
        if(x[i] <= pw[ii]):
            uu[i] = 1.0
    ...
    print('Saving data')      
    np.save('./Data/uFOM_xb=' + str(xb)+ '_pw=' + str(pw[ii]) + '_.npy',uFOM)
    ...
    plt.xlabel('x')
    plt.ylabel('u')
    plt.savefig('./Plots/burgers_Square_xb='+str(xb)+ '_pw='+str(pw[ii])+'_.png', dpi = 500, bbox_inches = 'tight')
    plt.show()
