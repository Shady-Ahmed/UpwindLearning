# -*- coding: utf-8 -*-
"""
Learning from past and present (LPP) closure for 1D Burgers problem with 
an initial condition of a pulse.
...
def create_training_data_lstm(features,labels, m, n, lookback):
    ...
    npt = int((n-1)/2)
    #npt = n-2
    xtrain = np.zeros((m-lookback,lookback,3*npt+2))
    for i in range(m-lookback):
        #a = np.copy(features[i,:])
        a = np.hstack([features[i,:],features[i+1,1:npt],features[i+1,-2] ])
        #a[1:-1] = np.copy(features[i+1,1:-1])
        for j in range(1,lookback):
            tmp = np.hstack([features[i+j,:],features[i+j+1,1:npt],features[i+j+1,-2] ])
...
xi = np.zeros((len(pw),ns+1,2*npt+2))
...
if training == 'true': 
    ...
    for i in range(4):
        features = xTrain[i,:,:]  #xe[i,:int(ns/tfreq),:]
        labels = yTrain[i,:,:]  #ye[i,:int(ns/tfreq),:]
    ...
    # Removing old models
    model_name = 'LSTM Model/LPP_Closure_'+str(npt)+'.h5'
    ...
    model = Sequential()
    #model.add(LSTM(10, input_shape=(lookback, features.shape[1]+npt), return_sequences=True, activation='tanh'))
    #model.add(LSTM(80, input_shape=(lookback, features.shape[1]+npt), return_sequences=True, activation='tanh'))
    #model.add(LSTM(40, input_shape=(lookback, features.shape[1]+npt), return_sequences=True, activation='tanh'))
    model.add(LSTM(10, input_shape=(lookback, features.shape[1]+npt), activation='tanh'))
...
for kk in range(len(pw)):
    ...
    xtest = np.zeros((1,lookback,3*npt+2))
    ...
    for i in range(lookback):
        tmp = np.hstack([xTest[i,:],xTest[i+1,1:npt],xTest[i+1,-2] ])
        tmp = tmp.reshape(1,-1)
    ...
    for i in range(lookback,ns+1):
        ...
        tmp = np.hstack([uu0[0:npt], xTest[i-1,npt:],uu[1:npt],xTest[i,-2] ])
        tmp = tmp.reshape(1,-1)
